...
} from '@arkade-os/boltz-swap/repositories/sqlite';
import {
  BoltzSwapProvider,
  isChainFailedStatus,
...
const SWAP_TASK_QUEUE_PREFIX = 'ark:swap-queue';
const SWAP_TASK_QUEUE_KEYS = [
  `${SWAP_TASK_QUEUE_PREFIX}:inbox`,
  `${SWAP_TASK_QUEUE_PREFIX}:outbox`,
  `${SWAP_TASK_QUEUE_PREFIX}:config`,
];

export async function clearLegacyNativeBoltzState(
  executor: SQLExecutor,
): Promise<void> {
  await new SQLiteSwapRepository(executor).clear();
  await AsyncStorage.multiRemove(SWAP_TASK_QUEUE_KEYS);
}
...
export class BoltzNativePaymentRail implements PaymentRail {
  readonly id = 'boltz';
  readonly supportedLayers = ['onchain', 'lightning'] as const;

  private constructor(
    private readonly wallet: IWallet,
    private readonly swaps: ExpoArkadeSwaps,
    private readonly swapProvider: BoltzSwapProvider,
  ) {}

  static async create(wallet: IWallet, executor: SQLExecutor): Promise<BoltzNativePaymentRail> {
    ...
    const swaps = await ExpoArkadeSwaps.setup({
      ...
      swapManager: true,
      background: {
        taskQueue: new AsyncStorageTaskQueue(AsyncStorage, SWAP_TASK_QUEUE_PREFIX),
        foregroundIntervalMs: 30_000,
      },
    });
    await swaps.refreshSwapsStatus().catch(() => {});
    return new BoltzNativePaymentRail(wallet, swaps, swapProvider);
  }
  ...
  async send(quote: PaymentQuote): Promise<PaymentRecord> {
    ...
    if (quote.layer === 'lightning') {
      const route = quote.request.routes.find(c => c.layer === 'lightning' && c.format === 'bolt11');
      ...
      const swap = (await this.swaps.getSwapHistory())
        ...
        .find(c => c.request.invoice === route.destination);
      ...
      if (swap) {
        ...
        return { ...toSubmarinePaymentRecord(fundedSwap), status: 'pending', refundable: false };
      }
      ...
      return {
        ...
        txid: result.txid,
        refundable: false,
        providerData: { destination: route.destination, sendAmountSats: result.amount },
      };
    }

    const route = quote.request.routes.find(c => c.layer === 'onchain');
    ...
    if (!refundTestMode) {
      void this.swaps.waitAndClaimBtc(fundedSwap)
        .then(async claimed => {
          Object.assign(fundedSwap, { completionTxid: claimed.txid, status: 'transaction.claimed' });
          await this.swaps.swapRepository.saveSwap(fundedSwap);
        })
        .catch(() => {});
    }
  ...
  async getPayment(paymentId: string): Promise<PaymentRecord | null> {
    const swap = (await this.swaps.getSwapHistory()).find(c => c.id === paymentId);
    if (swap && isPendingChainSwap(swap)) return this.recordForSwap(swap);
    if (swap && isPendingSubmarineSwap(swap)) return toSubmarinePaymentRecord(swap);
    if (swap && isPendingReverseSwap(swap)) return toReversePaymentRecord(swap);
    return null;
  }

  async listPayments(): Promise<PaymentRecord[]> {
    const swaps = await this.swaps.getSwapHistory();
    const chain = swaps.filter(isPendingChainSwap).map(toChainPaymentRecord);
    const submarine = swaps.filter(isPendingSubmarineSwap).map(toSubmarinePaymentRecord);
    const reverse = swaps.filter(isPendingReverseSwap).map(toReversePaymentRecord);
    return [...chain, ...submarine, ...reverse];
  }
  ...
  async refund(paymentId: string): Promise<PaymentRecord> {
    ...
    return (await this.getPayment(paymentId)) ?? (isPendingChainSwap(swap) ? toChainPaymentRecord(swap) : toSubmarinePaymentRecord(swap));
  }
